...
1. 再帰的に評価する仕組みに修正
   - 一気に指定した使用しているライブラリをハードコードしたけんすうのみしゅとくするのではなく、オプションで指定した数を同じコードで再帰的に分析するようにしたい
...
1. OSS の情報
   | oss_ID | oss 名 | oss 情報 | 各インサイト | 貢献回数 |
   | :--------: | :------: | :----: | :----------------: | :--------------------: |
   | A | B | C | D | E |
   | F | G | H | I | J |
   | K | L | M | N | O |
...
1. 疎結合
   - 評価軸・データ取得元を簡単に追加・削除できるようにしたい
     - そのために、ファイルもタスクごとではなく、評価軸ごとに分けた
1. 可読性
