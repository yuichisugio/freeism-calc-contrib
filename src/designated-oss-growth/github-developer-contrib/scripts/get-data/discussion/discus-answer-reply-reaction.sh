...
# 出力先のディレクトリを作成する
#--------------------------------------
readonly RAW_GET_DISCUSSION_ANSWER_REPLY_REACTION_DIR="${RESULT_GET_DISCUSSION_DIR}/raw-discus-answer-reply-reaction.jsonl"
readonly RESULT_GET_DISCUSSION_ANSWER_REPLY_REACTION_PATH="${RESULT_GET_DISCUSSION_DIR}/result-discus-answer-reply-reaction.json"
mkdir -p "$(dirname "$RESULT_GET_DISCUSSION_ANSWER_REPLY_REACTION_PATH")"
...
function get_discussion_answer_reply_reaction() {
  ...
  get_paginated_data_by_node_id \
    "$QUERY" \
    "$RAW_GET_DISCUSSION_ANSWER_REPLY_REACTION_DIR" \
    "$RESULT_GET_DISCUSSION_ANSWER_REPLY_REACTION_PATH" \
    "reactions" \
