...
#--------------------------------------
# pull requestの担当者のタイムラインを取得するファイル
#--------------------------------------

set -euo pipefail

#--------------------------------------
# pull requestの担当者のタイムラインを取得する関数
#--------------------------------------
function get_pull_request_timeline_assigned() {
  ...
  local QUERY
  local RAW_PATH="${RESULT_GET_PR_DIR}/raw-pr-timeline-assigned.jsonl"
  local RESULT_PATH="${RESULT_GET_PR_DIR}/result-pr-timeline-assigned.json"
  ...
  # データ取得後のRateLimitを出力
  get_ratelimit \
    "after:get-pull-request-timeline-assigned()" \
